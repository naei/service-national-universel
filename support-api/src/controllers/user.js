...
const ERROR_EMAILMISSING = "ERROR_EMAILMISSING";

(async () => {
  const arr = await SupportUserModel.find({});
  console.log("Arr", arr);
})();
...
//       return res
//         .status(200)
//         .send({ ok: true, token: value.token, user: supportUser });
//     } catch (error) {
//       capture(error);
...
// router.post(
//   "/",
//   passport.authenticate("support-user", {
//     session: false,
//     failWithError: true,
//   }),
//   async (req, res) => {
...
//       if (!email)
//         return res.status(400).send({ ok: false, code: ERROR_EMAILMISSING });
//       if (await SupportUserModel.findOne({ email }))
//         res
//           .status(400)
//           .send({ ok: false, code: ERRORS.USER_ALREADY_REGISTERED });
//       const referentUser = await ReferentModel.findOne({ email });
//       if (!referentUser)
//         res.status(400).send({ ok: false, code: ERRORS.USER_NOT_EXISTS });
//       const newUser = {
...
//       if (!user)
//         res.status(400).send({ ok: false, code: ERRORS.USER_NOT_FOUND });
//       const updatedUser = {};

//       if (req.body.hasOwnProperty("projectUserId"))
//         updatedUser.projectUserId = req.body.projectUserId;
...
//       if (req.body.hasOwnProperty("lastLoginAt"))
//         updatedUser.lastLoginAt = req.body.lastLoginAt;
//       if (req.body.hasOwnProperty("registerdAt"))
//         updatedUser.registerdAt = req.body.registerdAt;
//       if (req.body.hasOwnProperty("role")) updatedUser.role = req.body.role;
//       if (req.body.hasOwnProperty("filters"))
//         updatedUser.filters = req.body.filters;
